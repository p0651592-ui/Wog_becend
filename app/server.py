...
from app.db.models import AdminAuditLog, GameRound, PlayerStats, PromoCode, Transaction, User, Wallet
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif}
    ...
    .app{width:100%;max-width:1080px;display:flex;flex-direction:column;gap:14px}
    .topbar,.panel,.card{background:var(--card);border:1px solid var(--border);border-radius:var(--r);box-shadow:var(--shadow)}
    .topbar{padding:16px;display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap}
    .grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}
    .card{padding:14px}
    .label{font-size:11px;color:var(--muted);font-weight:800;text-transform:uppercase;letter-spacing:.6px}
    ...
    table{width:100%;border-collapse:collapse;min-width:780px;background:#101523}
    ...
    .btn{border:none;border-radius:12px;padding:10px 14px;font-weight:900;cursor:pointer;color:var(--text);background:var(--blue)}
...
<body>
  <div class="app">
    <div class="topbar">
      ...
      <div class="actions">
        <span class="btn secondary" id="admin-status">loading</span>
        <button class="btn secondary" id="refresh-btn" type="button">Обновить</button>
      </div>
    </div>

    <div class="grid">
      <div class="card"><div class="label">Пользователи</div><div class="value" id="metric-users">0</div><div class="muted">Всего аккаунтов</div></div>
      <div class="card"><div class="label">Активные</div><div class="value" id="metric-active">0</div><div class="muted">Статус active</div></div>
      <div class="card"><div class="label">Игры</div><div class="value" id="metric-rounds">0</div><div class="muted">Всего раундов</div></div>
      <div class="card"><div class="label">Баланс в системе</div><div class="value" id="metric-balance">0</div><div class="muted">Сумма WC по кошелькам</div></div>
    </div>
    ...
    <div class="layout">
      <div class="panel">
        <div class="actions" style="justify-content:space-between;align-items:center">
          <div>
            <h3>Игроки</h3>
            <p class="muted">Баланс, статус и роль. Действия доступны только администратору.</p>
          </div>
      ...
      <div class="panel">
        <div>
          ...
          <p class="muted">Начисление, списание, смена статуса и роли.</p>
        </div>
        <div class="form-row">
...
def on_startup() -> None:
    ...
    with next(get_db()) as session:
        PromoService.seed_default_promos(session)
        session.commit()
...
def redeem_promo(payload: PromoRedeemPayload, db: Session = Depends(get_db)) -> dict[str, Any]:
    user, _ = _ensure_user(db, payload.init_data)
    promo_result = PromoService.redeem(db, user, payload.code)
    db.commit()
    profile = StatsService.build_profile_payload(db, user)
    profile["text"] = StatsService.format_profile_text(profile)
    return {
        "status": "success",
        "message": f"Промокод {promo_result['code']} активирован: +{promo_result['reward_balance']} WC",
        "balance": promo_result["balance"],
        "reward_balance": promo_result["reward_balance"],
        "profile": profile,
    }
...
def spin_wheel_classic(payload: WheelClassicSpinPayload, db: Session = Depends(get_db)) -> dict[str, Any]:
    ...
    if payload.bet > 0:
        WalletService.place_bet(
            ...
            user.id,
            payload.bet,
            meta={"game": "wheel_classic", "bet_type": payload.bet_type, "number": payload.number},
        )
